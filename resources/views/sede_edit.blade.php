...
@section('content')
    ...
    <div class="container-fluid">
        <h4>Editar cadastro da sede</h4>
        <hr>
        <div class="card mt-3" style="padding: 10px">
            <form method="POST" action="{{ route('sede.update', $sede_filtro->id) }}">
                @csrf
                @method('PUT')
                <div class="row form-group">
                    <div class="col-md-6">
                        <label for="sede">Nome da nova Sede</label>
                        <input type="text" name="sede" id="sede" class="form-control" required value="{{$sede_filtro->sede}}">
                    </div>
                    <div class="col-md-6">
                        <label for="diretor">Nome do Diretor</label>
                        <input type="text" name="diretor" id="diretor" class="form-control" required value="{{$sede_filtro->diretor}}">
                    </div>
                </div>
